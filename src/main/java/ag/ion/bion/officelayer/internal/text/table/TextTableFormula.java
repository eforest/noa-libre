...
public class TextTableFormula implements IFormula {
    ...
    public static final String[] operators =
        new String[] { PLUS, MINUS, DIVIDE, MULTIPLY, ROUND, PERCENT, SQRT, POW, MEAN };
            ...
            if ( argumentBez.indexOf( ":" ) != -1 ) {
                ...
                textTableFormulaExpression
                                          .setExpression(
                                              textTableFormulaExpression
                                                                        .getExpression()
                                                                        .replaceFirst( argumentBez, helpString ) );
            }
                ...
                textTableFormulaExpression
                                          .setExpression(
                                              textTableFormulaExpression.getExpression().substring( 0, index ) + "("
                                                  + pipe + ")"
                                                  + textTableFormulaExpression
                                                                              .getExpression()
                                                                              .substring(
                                                                                  index + argumentBez.length() ) );
            }
            else {
    ...
    private void getMaxMin(boolean maximum, String formula, int formulaIndex) {
        ...
        if ( index != -1 ) {
            ...
            for ( int i = 0; i < help.length(); i++ ) {
                if ( help.charAt( i ) == '|' ) {
                    helpIndex = maxMin.length()
                        + ( help.lastIndexOf( "|" ) + help.substring( help.lastIndexOf( "|" ) ).indexOf( ">" ) ) + 1;
                    found = true;
                    String helpString = help.substring( oldIndex, i ).trim();
                    if ( helpString.indexOf( "<" ) != -1 ) {
                        helpString =
                            helpString.substring( helpString.indexOf( "<" ), helpString.indexOf( ">" ) + 1 ).trim();
                        if ( helpString.indexOf( ":" ) != -1 ) {
                        ...
                        maxArguments.addArgument( argument );
                        helpIndex = helpIndex + help.indexOf( new Double( argument.getValue() ).toString() )
                            + new Double( argument.getValue() ).toString().length();
                    }
                    oldIndex = i;
                }
            }
            ...
            textTableFormulaExpression
                                      .setExpression(
                                          textTableFormulaExpression
                                                                    .getExpression()
                                                                    .substring( 0, formulaIndex + index )
                                              + maxMin + ( index + formulaIndex )
                                              + textTableFormulaExpression
                                                                          .getExpression()
                                                                          .substring(
                                                                              formulaIndex + index + helpIndex ) );
