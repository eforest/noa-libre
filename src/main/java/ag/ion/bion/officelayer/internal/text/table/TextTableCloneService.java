...
public class TextTableCloneService extends AbstractCloneService {
    ...
    // ----------------------------------------------------------------------------
    /**
     * Clones the table to the given position and then returns a reference. If the position is null, then the clone is
     * placed at the end of the document.
     * 
     * @param range append this variable indicates if the clone shiuld be appended or not
     ...
     */
    public IClonedObject cloneToPosition(IDestinationPosition range, PropertyKeysContainer propertyKeysContainer)
    ...
    // ----------------------------------------------------------------------------
    /**
     * Clones the chosen table to the given position and then returns a reference This method also enables to adopts the
     * content of the table (the default is to adopt, otherwise the paramter has to be set to false)
     * 
     * @param range append this variable indicates if the clone should be appended or not
     ...
     * @param createSpace indicates if a new paragraph should be constructed to create some space between the old and
     *            the new textElement
     ...
     * @author Markus Krüger
     */
    private IClonedObject clonePreprocessor(IDestinationPosition position, boolean adoptContent, boolean createSpace,
        ...
        try {
            ...
            if ( propertyKeysToCopy != null ) {
                ...
                ( (IPropertyStore) tablePropertyStore )
                                                       .getProperties()
            ...
            for ( int rows = 0; rows < rowArrayLength; rows++ ) {
                ...
                for ( int columns = 0; columns < columnArrayLength; columns++ ) {
                    ...
                    CloneDestinationPosition destinationCell =
                        new CloneDestinationPosition( textTableCell, textTableCell.getClass() );
    ...
    // ----------------------------------------------------------------------------
    /**
     * Gets the properties form the table and the cells and then stores them in the apropriate objects designed for
     * them.
     * 
