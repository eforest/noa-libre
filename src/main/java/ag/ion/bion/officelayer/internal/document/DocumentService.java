...
public class DocumentService implements IDocumentService {
    ...
    // ----------------------------------------------------------------------------
    /**
     * Constructs new document in the submitted frame. <br>
     * <br>
     * Use one of the types: <br>
     * <br>
     ...
     * @author Andreas Bröker
     */
    public IDocument constructNewDocument(IFrame frame, String documentType, IDocumentDescriptor documentDescriptor)
        throws NOAException {
        try {
    ...
    // ----------------------------------------------------------------------------
    /**
     * Constructs new hidden document of the submitted type. <br>
     * <br>
     * Use one of the types: <br>
     * <br>
     * <code>IDocument.WRITER</code><br>
     ...
     * @throws DocumentException if the document can not be loaded or the URL does not locate an OpenOffice.org document
     * @author Andreas Bröker
     */
     ...
     * @param documentDescriptor document descriptor to be used
     * @return loaded document
     * @throws NOAException if the document can not be loaded or the URL does not locate an OpenOffice.org document
     * @author Andreas Bröker
     * @date 02.07.2006
     */
    ...
    // ----------------------------------------------------------------------------
    /**
     * Loads document on the basis of the submitted stream. May be <b>significantly slower</b> than the load* variants
     * which take an URL String as parameter.
     * 
    ...
    // ----------------------------------------------------------------------------
    /**
     * Loads document on the basis of the submitted stream. <b>The document has no location and can not be stored with
     * the store() method of the <code>IPersistenceService</code>. Furthermore OpenOffice.org can not recognize if the
     * document is already open - therefore the document will be never opened in <code>ReadOnly</code> mode.</b> May be
     * <b>significantly slower</b> than the load* variants which take an URL String as parameter.
     * 
     ...
     * @author Andreas Bröker
     */
    public IDocument loadDocument(IFrame frame, InputStream inputStream, IDocumentDescriptor documentDescriptor)
        throws DocumentException {
     ...
     * @author Andreas Bröker
     */
    public IDocument loadDocument(IOfficeProgressMonitor officeProgressMonitor, InputStream inputStream,
     ...
     * @author Andreas Bröker
     */
    public IDocument loadDocument(IOfficeProgressMonitor officeProgressMonitor, IFrame frame, InputStream inputStream,
                                  IDocumentDescriptor documentDescriptor)
        throws DocumentException {
        XComponent xComponent = null;
        ...
        try {
            ...
            // officeProgressMonitor.beginTask(Messages.getString("DocumentService_monitor_message_preparing_loading"),
            // IOfficeProgressMonitor.WORK_UNKNOWN); //$NON-NLS-1$

            ByteArrayXInputStreamAdapter byteArrayToXInputStreamAdapter =
                new ByteArrayXInputStreamAdapter( inputStream, officeProgressMonitor );
            properties[0] =
                new PropertyValue( "InputStream", -1, byteArrayToXInputStreamAdapter, PropertyState.DIRECT_VALUE ); //$NON-NLS-1$
            properties =
                DocumentDescriptorTransformer.documentDescriptor2PropertyValues( properties, documentDescriptor );
     ...
     * @return loaded document
     * @throws DocumentException if the document can not be loaded, the URL does not locate an OpenOffice.org document
     *             of the submitted frame or URL is not valid
     * @author Andreas Bröker
     */
     ...
     * @return loaded document
     * @throws DocumentException if the document can not be loaded, the URL does not locate an OpenOffice.org document
     *             or the submitted frame or URL is not valid
     * @author Andreas Bröker
     */
                ...
                document = DocumentLoader
                                         .loadDocument(
                                             ...
                                             DocumentDescriptorTransformer
                                                                          .documentDescriptor2PropertyValues(
                                                                              documentDescriptor ) );
            }
            else {
                document = DocumentLoader
    ...
    // ----------------------------------------------------------------------------
    /**
     * Checks if the OpenOffice.org instance reached the number of maximum open documents and throws an exception if
     * this is the case.
     * 
     * @param serviceProvider the serviceProvider to be used
        ...
        try {
            if ( serviceProvider != null
                ...
                throw new NOAException(
                    "The maximum number of opend documents was reached (Maximum number is " //$NON-NLS-1$
    ...
    private IDatabaseDocument constructDatabaseDocument() throws NOAException {
        try {
            Object dataSource = officeConnection
            ...
            XDocumentDataSource documentDataSource =
                (XDocumentDataSource) UnoRuntime.queryInterface( XDocumentDataSource.class, dataSource );
    ...
    private XComponentLoader constructComponentLoader() throws Exception {
        Object oDesktop = officeConnection
                                          .getXMultiComponentFactory()
                                          .createInstanceWithContext(
